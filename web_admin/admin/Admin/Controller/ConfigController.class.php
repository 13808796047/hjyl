...
class ConfigController extends AdminController
{
    ...
    {
        if (!is_numeric(I('clearMemberCoin'))) {
            $this->error('金额未填');
        }

        if (!is_numeric(I('clearMemberDate'))) {
            $this->error('天数未填');
        }
    ...
    {
        $bank = M('member_bank');
        $bank->enable = I('enable');
        $bank->where(array('id' => I('id')))->save();

        $this->success('修改成功');
    }
    final public function delBank()
    {
    ...
    //彩种设置
    final public function lotteryList()
    {
        if (IS_POST) {
    ...
    //玩法设置
    final public function playList()
    {
        if (IS_POST) {
    ...
    //更改group状态
    final public function switchPlayedGroupStatus()
    {
        if (IS_POST) {
            $data['id'] = I('id');
            if (I('enable')) {
    ...
    //更改played状态
    final public function switchPlayedStatus()
    {
        if (IS_POST) {
            $Config = D('played');
            $data = $Config->create();

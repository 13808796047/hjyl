...
;(function(host, name, Event, $, undefined){
	...
	var pros = {
		...
		reSetPosition: function(){
			var me = this,position = me['defConfig']['isFixed'] ? 'fixed' : 'absolute';
			position = util.isIE6 ? 'absolute' : position;
			me.dom.removeAttr('style');
			me.dom.css({
				'z-index': me['defConfig']['zIndex'],
				'position': position,
				'display': 'none'
			});
		},
		...
		setConfirmName: function(text){
			var me= this;
			me.getConfirmButtonDom().html(text + '<b class="btn-inner"></b>');
		},
		//修改取消按钮名称
		setCancelName: function(text){
			var me= this;
			me.getCancelButtonDom().html(text + '<b class="btn-inner"></b>');
		},
		//修改关闭按钮名称
		setCloseName: function(text){
			var me= this;
			me.getCloseButtonDom().html(text + '<b class="btn-inner"></b>');
		},
		//显示确认按钮对象
		...
		showWaringIcon: function(){			
			var me= this;
			me.getContainerDom().find('.ico-waring').show();
		},

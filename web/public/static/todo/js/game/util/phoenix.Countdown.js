	...
	var pros = {
		...
		_countFun : function(now) {
			...
			this.timeFun = setInterval(function() {
				...
				// KM: 全局監控時間倒數並撥放音效
				// 每秒檢查似乎很消耗效能 QQ....應該有更好的實作法 , 而且前端 initbell 事件應該寫在 phoenix 內部 不暴露出來,再加上 uglify => perfect..(當然都只是防君子...)
				var audioPlayer = isIE ? document.getElementById('countdownSoundForIE8') : document.getElementById('countdownSound');
				if ($.cookie('knell') == 'on' && $('#toogle_bell').hasClass('bell_on')) {
					if (time.h == 0 && time.m == 0 && time.s < 6) {
						audioPlayer.play();
					}
				} else {
					audioPlayer.pause();
		...
		redRessTime: function(){
			...
			me.timeload = $.ajax({
				...
				data: {'curmid': host.Games.getCurrentGame().getCurrentMid()},
				cache:false,
				type: 'GET',
				dataType: 'json'
			})
			.done(function(data) {
				...
				if(Number(data['isSuccess']) == 1){
					...
					me.fireEvent('afterSuccessRedRessTime', data);
				}

!(function () {
  ...
  var t = function (t) {
    ...
    e.getCurrentGame().addEvent("afterSwitchGameMethod", function () {
      r
        ? (phoenix.Games.SSC.getInstance().removeFakeBallDom(),
          (r = !1),
          o.historyBall &&
            setTimeout(function () {
              e.getCurrentGame().getCurrentGameMethod().refreshHistory(o);
            }, 100))
        : e.getCurrentGame().getCurrentGameMethod().refreshHistory();
    }),
      e.getCurrentGameTypes().addEvent("endChange", function () {
        a &&
          (phoenix.Games.SSC.getInstance().removeFakeMenuDom(),
          e.getCurrentGameTypes().containerShow(),
          (a = !1));
      }),
      e.getCurrentGame().addEvent("changeDynamicConfig", function (t, n) {
        if (
          ("undefined" != typeof n.balance &&
            $("#J-balance-value").html(n.balance),
          n.lastballs)
        ) {
          var a = n.lastballs.split(",");
          $("#J-lotter-info-number").text(n.number),
            $("#J-lotter-info-lastnumber").text(n.lastnumber),
            $("#J-lottery-info-balls")
              .find("em")
              .each(function (e) {
                this.innerHTML = a[e];
              }),
      ...
      e.getCurrentGameTypes().addEvent("endShow", function () {
        $.support.leadingWhitespace
          ...
          : setTimeout(function () {
              e.getCurrentGameTypes().changeMode(
                e
                  .getCurrentGame()
                  .getGameConfig()
                  .getInstance()
      ...
      setTimeout(function () {
        $("body").append(
          '<iframe src="' +
            e.getCurrentGame().getGameConfig().getInstance().getUploadPath() +
            '" name="check_file_frame" style="display:none;"></iframe>'
        ),
          phoenix.GameTrace.getInstance();
      }, 0),
      e.getCurrentGame().addEvent("afterSuccessGetLastBall", function (t, n) {
        var a = n.ball.split(","),
          r = n.period;
        try {
      ...
      i.addEvent("afterSuccessRedRessTime", function (t, n) {
        n.availablebalance &&
          e.getCurrentGame().setBalanceValue(n.availablebalance);
      }),
      e.getCurrentGameSubmit().addEvent("afterSubmitSuccess", function () {
              ...
              (o = setTimeout(function () {
                (n = t.find("a").width()),
                  ...
                  $(window).scrollTop() > r
                    ? t.show()
                    : (t.hide(),
                      t.hasClass("countdown-current") ||
                        t.addClass("countdown-current"));
              }, 30));
          }),
          t.find("a").bind("click", function () {
            t.hasClass("countdown-current")
              ? t.removeClass("countdown-current")
              : t.addClass("countdown-current");
          }),
          i.addEvent("afterShowTime", function (n, a, r) {
            var o = r.checkNum(a.m) + "",
              i = r.checkNum(a.s) + "";
            e.html(o + ":" + i), t.find("strong").html(o + ":" + i);
          });
      })(),
      i.addEvent("afterTimeFinish", function () {
        var t = e.getCurrentGameMessage();
        e
          .getCurrentGame()
          .delCurrentPeriod(e.getCurrentGame().getDynamicConfig()),
      ...
      $(".prompt")
        .on("mouseenter", ".example-button", function () {
          var e = document
      ...
      (function () {
        var e = $("#J-game-chart"),
          t = function () {
            var t =
              "/newgame_play.html?curmid=" +
              window.GamesInitData.curmid +
              "&flag=historyBalls&size=10";
            $.ajax({
              url: t,
              dataType: "html",
              success: function (t) {
              ...
              error: function (t, n) {
                e.find(".inner-content").html(
                  translate.FailToGetNumberWording + t.responseText
                );
              },
            });
          };
        $(".chart-switch").bind("click", function () {
          $(".control-btn").removeClass("current-control"),
            e.parent().hasClass("game-chart")
              ? e.parent().removeClass()
              : ($(this).addClass("current-control"),
                e.parent().removeClass(),
                t(),
                e.parent().addClass("game-chart"));
        });
      })(),
      $("#J-add-order").click(function () {
        e.getCurrentGameOrder().add(
      ...
      $("#J-fresh-balance").on("click", function () {
        e.getCurrentGame().getBalanceFormServer();
      }),
      ...
      $("body").on("click", "#J-close-fnarea", function () {
        $(this).parent().removeClass(),
          $(".control-btn").removeClass("current-control");
      }),
      $("#J-history-control").on("click", ".menu-button-title a", function () {
      ...
      $("#J-history-control").on("click", ".details-lottery", function () {
        $(this).next().hide().show();
      }),
      $("#J-balls-statistics-lockMultiple, .icon-unlock, .icon-lock").on(
        "click",
        function (e) {
          var t = $("#J-balls-statistics-panel"),
            n = $("#J-balls-statistics-lockMultiple");
          t.find(".icon-unlock")[0]
            ? (n.prop("checked", !0),
              t
                .find(".icon-unlock")

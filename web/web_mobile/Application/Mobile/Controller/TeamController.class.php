...
//    }
    ...
    public final function search()
    {
        ...
        switch ($para['utype']) {
            ...
            case 3:
                // 所有下级
                $map['parents'] = ['like', '%,' . $this->user['uid'] . ',%'];
                break;
            default:
                //所有人
                $map['parents'] = ['like', "%," . $this->user['uid'] . ",%"];
...
//			$this->display('Team/member-list');
    ...
    public final function memberDetail()
    {
        ...
        $sonFanDianMax = M('members')->where(['isDelete' => 0, 'parentId' => I('uid')])->field('max(fanDian) sonFanDian, max(fanDianBdw) sonFanDianBdw')->find();
...
//        }
        if(I('username') && I('username') != '用户名') {
            ...
            // 只要符合用户名且是自己所有下级的都可查询
            // 用户名用模糊方式查询
            $where['parents'] = array('like', "%," . $user_id . ",%");
            $where['username'] = array('like', '%' . strtolower(trim(I('username'))) . '%');
...
//            $where['m.parents']  = array('like', "%," . $this->user['uid'] . ",%");
    ...
    public final function team()
    {
        ...
        $teamAll2 = M('members')->where(['isDelete' => 0, 'parentId' => $this->user['uid']])->field('count(uid) count')->find();

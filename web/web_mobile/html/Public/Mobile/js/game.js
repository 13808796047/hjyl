$(function () {
  ...
  $(".dmtm a.code").live("click", function (event) {
    ...
    if (
      $(".code.checked[value=" + this.innerHTML + "]", $dom).not(this).length ==
      1
    ) {
      $this.removeClass("checked");
  ...
  $(".zhixu115 a.code").live("click", function (event) {
    ...
    var $dom = $(".zhixu115");
    $(".code.checked[value=" + this.innerHTML + "]", $dom).removeClass(
      "checked"
    );
    $this.addClass("checked");
  });
...
function gamePostCode() {
  ...
  wait();
  var actionNo = $.parseJSON(
    $.ajax("/index.php?s=/mobile/game/getNo/type/" + game.type, {
      async: false,
    }).responseText
  );
  destroyWait();
        ...
        确定购买: function () {
          ...
          $.ajax("/index.php?s=/mobile/game/postCode", {
            ...
            complete: function (xhr, textStatus) {
              ...
              if (errorMessage)
                gamePostedCode(decodeURIComponent(errorMessage));
            },
          });
        },
        取消购买: function () {
...
function gameAddCode(code) {
  wait();
  var actionNo = $.parseJSON(
    $.ajax("/index.php?s=/mobile/game/checkBuy", { async: false }).responseText
  );
  destroyWait();
  ...
  try {
    ...
    if ($wei.length) {
      if ($(":checked", $wei).length != weiCount)
        throw "请选择" + weiCount + "位数！";
...
function winjinAlert(tips, style, minH) {
  $("#wanjinDialog")
    .html('<span class="ui-wjicon-' + style + '"></span><b>' + tips + "</b>")
    .dialog({
      title: "温馨提示",
      modal: true,
      resizable: false,
...
function gameSetPl(value, flag, fanDianBdw) {
  ...
  if (fanDianBdw) {
    ...
    else $dom.html("<option value=" + fan2 + ">" + fan2 + "</option>");
  }
}
      ...
      if (!data) {
        ...
        try {
          ...
          if (ctype == "g1") {
            ...
            if ($dom.find(".feipan"))
              $dom.find(".feipan").html("快乐飞盘：<em>" + $feipan + "</em>");
          } else if (ctype == "g2") {
            //快3
            $(".kjhao li").each(function (i) {
              times -= 500;
              setTimeout(
                "setKjing1(" + i + ",'gr_ks gr_ks" + hao[i] + "')",
                times
              );
            });
          } else {
          ...
          if (
            typeof $("#wanjinDialog").dialog("isOpen") == "object" ||
            $("#wanjinDialog").dialog("isOpen")
          ) {
...
function getQiHao() {
  $.getJSON(
    "/index.php?s=/mobile/game/getQiHao/type/" + game.type,
    function (data) {
...
function selectGroup(self) {
  var html =
    '已选择了<font id="count">0</font>注,共￥<font id="amount">0.00</font>元';
  $("#count-amount").html(html);
